...
static void upsert_task_from_payload(const char *p, uint32_t len) {
    ...
    if(len<4) return;
    uint32_t assign_len;
    memcpy(&assign_len,cur,4);
    assign_len=ntohl(assign_len); cur+=4; len-=4;

    if(assign_len>1024 || assign_len>len) return ;
    char assigned_to[128]={0};
    if(assign_len) {
        memcpy(assigned_to,cur,assign_len<127 ? assign_len:127);
        cur+=assign_len; len -=assign_len;
    }

    if(len<4) return;
    uint32_t priority_len;
    memcpy(&priority_len,cur,4);
    priority_len=ntohl(priority_len); cur+=4; len-=4;
    if(priority_len>len) return;
    char priority[16]={0};
    if(priority_len)
    {
        memcpy(priority,cur,priority_len<15 ? priority_len :15);
        cur+=priority_len;
        len-=priority_len;
    }
    ...
    if (found) {
        ...
        strncpy(found->assigned_to,assigned_to,sizeof(found->assigned_to)-1);
        strncpy(found->priority,priority,sizeof(found->priority)-1);
    } else {
        ...
        strncpy(n->assigned_to,assigned_to,sizeof(n->assigned_to)-1);
        strncpy(n->priority,priority,sizeof(n->priority)-1);
        n->next = tasks_head;
        tasks_head = n;
    }
    pthread_mutex_unlock(&tasks_mutex);
...
static void load_tasks_from_sync_payload(const char *p, uint32_t len) {
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        memcpy(&assign_len,cur,4);
        assign_len=ntohl(assign_len); cur+=4; len-=4;
        if(assign_len>len) break;
        char assigned_to[128]={0};
        if(assign_len>0){
            memcpy(assigned_to,cur,assign_len<sizeof(assigned_to)-1 ?assign_len:sizeof(assigned_to)-1);
            cur+=assign_len;
            len-=assign_len;
        }
...
static void get_task_stats(int *total,int *done,int *pending,int *high){
    *total = 0;
    *done = 0;
    *pending = 0;
    *high = 0;
...
static void draw_ui() {
    ...
    get_task_stats(&total,&done,&pending,&high);
    mvwprintw(mainwin,1,2,"Tasks:%d | Done:%d | Pending:%d | High:%d",total,done,pending,high);
    ...
    while (cur && row <= maxrows) {
        ...
        if (idx == selected_index && show_info) {
            snprintf(
                ...
                "%s [%c] %s [%s] (@%s) | assigned:%s | by:%s @ %s | due:%s",
                cur->id,
                cur->done ? 'x' : ' ',
                cur->title,
                cur->priority[0] ? cur->priority: "MEDIUM",
                cur->assigned_to[0]? cur->assigned_to: "none",
                cur->assigned_to[0]? cur->assigned_to : "none",
    ...
    pthread_mutex_lock(&toast_mutex);
    if(toast_until && time(NULL)>toast_until)
    {
        toast_msg[0]='\0';
        toast_until=0;
    }
...
static void send_add_request(const char *title, long due_at,const char* assigned_to,const char *priority) {
    uint32_t tlen = (uint32_t)strlen(title);
    uint32_t alen = (uint32_t)strlen(assigned_to);
    uint32_t plen = (uint32_t)strlen(priority);

    uint8_t due_present = due_at > 0 ? 1 : 0;
    uint32_t payload_len = 4 + tlen + 1 + 4 + 4+alen + 4 + plen;
    char *buf = malloc(payload_len);
    char *cur = buf;
    uint32_t tmp;
    ...
    *cur++ = due_present ? 1 : 0;
    ...
    tmp=htonl(alen); memcpy(cur,&tmp,4); cur+=4;
    memcpy(cur,assigned_to,alen); cur+=alen;
    tmp=htonl(plen);
    memcpy(cur,&tmp,4); cur+=4;
    memcpy(cur,priority,plen); cur+=plen;
...
static void prompt_add() {
    ...
    move(LINES - 2, 1); clrtoeol();
    mvprintw(LINES - 2, 1, "Assign to (optional): ");
    refresh();
    getnstr(assigned_to,sizeof(assigned_to)-1);
    move(LINES-2,1);
    clrtoeol();
    mvprintw(LINES-2,1,"Priority(LOW/MEDIUM/HIGH)");
    refresh();
    getnstr(priority,sizeof(priority)-1);
    if(strlen(priority)==0) strcpy(priority,"MEDIUM");
    noecho(); curs_set(0);
    long due_epoch = parse_date_to_epoch(duebuf);
    send_add_request(titlebuf, due_epoch,assigned_to,priority);
    draw_ui();
}
