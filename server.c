...
static char *task_from_stmt_payload(sqlite3_stmt *stmt, uint32_t *out_len) {
    ...
    const unsigned char *assigned_to = sqlite3_column_text(stmt,3);
    const unsigned char *priority = sqlite3_column_text(stmt,4);
    int version = sqlite3_column_int(stmt,5);
    const unsigned char *updated_by = sqlite3_column_text(stmt,6);
    int updated_at = sqlite3_column_int(stmt,7);
    int due_at =
    sqlite3_column_type(stmt,8)==SQLITE_NULL ?
    0 : sqlite3_column_int(stmt,8);
    ...
    uint32_t assign_len =assigned_to ?(uint32_t)strlen((const char*)assigned_to):0;
    uint32_t priority_len= priority ? (uint32_t)strlen((const char*)priority) : 0;
    // payload fields:
    // [u32 idlen][id][u32 titlelen][title][u8 done][u32 version][u32 updated_by_len][updated_by][u32 updated_at][u32 due_at]
    uint32_t payload_len =4 + idlen +4 + titlelen +1 +4 + assign_len +4+priority_len+4  +4 + updby_len +4 +4;
    ...
    *cur++ = done ? 1 : 0;
    tmp=htonl(assign_len);
    memcpy(cur,&tmp,4);cur+=4;

    if(assign_len){
        memcpy(cur,assigned_to,assign_len);
        cur+=assign_len;
    }

    tmp=htonl(priority_len);
    memcpy(cur,&tmp,4); cur+=4;
    if(priority_len){
        memcpy(cur,priority,priority_len);
        cur+= priority_len;
    }
...
static void send_sync_full(int fd) {
    ...
    const char *q = "SELECT id, title, done,assigned_to,priority, version, updated_by, updated_at, due_at FROM tasks ORDER BY id ASC;";
...
static char *server_add_task(const char *title, int due_at,
 const char *username,const char* assigned_to,const char *priority) {
    pthread_mutex_lock(&db_mutex);
    const char *sql =
    "INSERT INTO tasks(title, done, assigned_to, priority, version, updated_by, updated_at, due_at) "
    ...
    sqlite3_bind_text(stmt,2, assigned_to,-1,SQLITE_TRANSIENT );
    sqlite3_bind_text(stmt,3,priority,-1,SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt,4,username ? username : "",-1,SQLITE_TRANSIENT);
    if(due_at>0)
        sqlite3_bind_int(stmt,5,due_at);
    else 
    ...
    const char *q2 ="SELECT id, title, done, assigned_to, priority,version, updated_by, updated_at, due_at " "FROM tasks WHERE id = ?;";
        ...
        if (type == MSG_ADD_TASK) {
            if (payload2 && payload_len2 >= 4) {
                ...
                if (cur + tlen <= end && cur + tlen + 1 + 4 <= end) {
                    ...
                    uint32_t assigned_len = ntohl(*(uint32_t*)cur); cur += 4;
                    char assigned_to[128]="";
                    memcpy(assigned_to,cur,assigned_len<127 ?assigned_len:127);
                    cur+=assigned_len;

                    uint32_t priority_len=ntohl(*(uint32_t*)cur); cur+=4;
                    char priority[16]="";
                    memcpy(priority,cur,priority_len<15 ? priority_len:15);
                    cur+= priority_len;

                    char *task_json = server_add_task(titlebuf, due_present ? (int)due_at : 0, username,assigned_to,priority);
                    if (task_json) {
                        free(task_json);
                        pthread_mutex_lock(&clients_mutex);
